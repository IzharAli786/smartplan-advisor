import { z } from "zod";

/** The two roles (§3). Account creation is super_admin-only — enforced server-side. */
export const ROLES = ["super_admin", "advisor"] as const;
export const roleSchema = z.enum(ROLES);
export type Role = (typeof ROLES)[number];

/** Roles with full operational visibility (commission, all advisors, reports).
 *  The manager role was removed — full visibility is now the super admin's alone. */
export function isManagerial(role: Role): boolean {
  return role === "super_admin";
}

/** Only super_admin may create accounts or manage other admins (§3.2, §11.4). */
export function canCreateUsers(role: Role): boolean {
  return role === "super_admin";
}

/** Who a given role is allowed to edit/deactivate. */
export function canEditUser(actor: Role, _target: Role): boolean {
  return actor === "super_admin";
}

/** Commission figures must never reach an advisor (§11.1). */
export function canSeeCommission(role: Role): boolean {
  return isManagerial(role);
}
